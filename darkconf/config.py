...
def set_annotations_items(cls):
    for name, value in cls.__annotations__.items():
        ...
        if hasattr(value, "__origin__") and value.__origin__ == EnvVar:
            ...
            # TODO: Convert the value if needed
    ...
    for name, (typ, default_value) in self._default_config.items():
        if name in kwargs:
            ...
            if not isinstance(value, typ) and not (
                ...
                raise TypeError(f"Expected type {typ} for {name}, got {type(value)}")
            setattr(self, name, value)
        elif default_value is not None:
            setattr(self, name, default_value)
        elif isinstance(typ, type(typing.Union)) and type(None) in typing.get_args(typ):
            setattr(self, name, None)
        else:
            raise ValueError(f"Missing value for {name}, and no default provided")
...
def config(_cls=None, *, stage: typing.Optional[stages.Stage] = None):
    def wrap(cls):
        ...
        apply_parent_config(cls)
        set_annotations_items(cls)

        def __init__(self, **kwargs):
            config_init(self, **kwargs)
